...
function statistics_summary(list)
    local format = "%2s%-16s%16s%-10s%16s%-14s%-23s%-18s"
    ...
    show(string.format("%2s统计概览（%19s ~ %19s）%30s", "", time.todate(list.begin_time, "%Y-%m-%d %H:%M:%S"), time.todate(list.end_time, "%Y-%m-%d %H:%M:%S"), ""), "olivedrab", "ivory")
    show(string.format("%2s%-24s%-24s%-24s%-20s", "", "重置次数："..tostring(list.reset), "重连次数："..tostring(list.connect), "死亡次数："..tostring(list.death), "发呆次数："..tostring(list.idle)), "yellow", "black")
    show(string.format("%2s%-16s%20s%-12s%20s%-16s%-30s%-26s", "", "任务", "获得经验", " / 效率", "获得潜能", " / 效率", "总用时（占比）", "完成数（成功率）"), "white", "dimgray")
