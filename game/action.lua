...
function wait_no_busy(action)
    ...
    local l = wait_line((action or "")..";eat busy", 30, {Gag=true}, 10,
        "^你正忙着呢。$|^你身上没有这样东西。$", "^> $")
    if l == false then
        return wait_no_busy_return(wait_no_busy(action))
    end
    if l[1][0] == "你身上没有这样东西。" then
...
function look_dir(dir)
    ...
    local l = wait_line("look "..dir, 30, nil, 10,
        "^你要看什么？$|"..
        "^\\S+\\s+-\\s+$")
    if l == false then
...
function one_step_exec(dir)
    ...
    local l = wait_line(dir, 30, {StopEval=true}, 10,
        "^什么\\?$|^这个方向没有出路。$|"..
        "^你的动作还没有完成，不能移动。$|"..
        "^糟糕，你逃跑失败了！$|"..
        "^(\\S+)\\s+-\\s+")
    ...
    elseif l[0] == "什么?" or l[0] == "这个方向没有出路。" then
        if wait_line("look", 30, nil, 10, "^\\S+\\s+-\\s+$", "^> $") == false then
            return -1
        end
...
function take(drug)
    ...
    local l = wait_line("fu "..drug, 30, {StopEval=true}, 10,
        "^你(?:吃|服)下一\\S+|"..
        "^你敷上一副蝉蜕金疮药，顿时感觉伤势好了不少。$|"..
        "^你已经吃过一颗了，多吃无益。$|"..
        "^什么\\?$|"..
        "^你要服用什么？$")
    if l == false then
    ...
    elseif l[0] == "什么?" then
        ...
        run_i()
        if string.match(l[0], "吃下") or string.match(l[0], "服下") or string.match(l[0], "敷上") then
...
function aquire_other(method, list)
    ...
    local l = wait_line(method, 30, {StopEval=true}, 10,
