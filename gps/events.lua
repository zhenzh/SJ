...
function faint()
    ...
    var.go.pause = function()
        ...
        local l = wait_line(nil, 180, {StopEval=true}, 20,
            "^慢慢地一阵眩晕感传来，你终于又有了知觉....$|"..
            "^鬼门关 - $")
...
function yell_boat()
    ...
    local l = wait_line("yell boat", 30, {StopEval=true}, 20,
        "^什么\\?$|"..
        "^你使出吃奶的力气喊了一声：“船家”$|"..
    ...
    elseif var.go.pause ~= nil then
        ...
        l = wait_line("enter", 30, {StopEval=true}, 20,
            "^\\S+\\s+- |"..
            "^你的眼前一黑，接著什么也不知道了....$|"..
            "^你已经精疲力尽，动弹不得。$|"..
            "^什么\\?$|"..
...
function yell_boat_wait()
    ...
    var.go.pause = true
    local l = wait_line(nil, 1, {StopEval=true}, 20,
...
function knock_lou()
    message("info", debug.getinfo(1).source, debug.getinfo(1).currentline,
            "函数［ knock_lou ］")
    local l = wait_line("knock lou", 30, nil, 20,
        "^你用力的敲了敲铜锣。$",
        "^一个大竹篓缓缓的(?:升了起|降了下)来。$|"..
        "^吊篮还没有离开呢，快上去吧。$"..
        "^吊篮正在运转过程中，请稍候。$")
...
function leave_transport()
    ...
    l = wait_line("out", 30, {StopEval=true}, 20,
        "^\\S+\\s+- |"..
        "^你的眼前一黑，接著什么也不知道了....$|"..
        "^你已经精疲力尽，动弹不得。$|"..
        "^鬼门关 - $")
...
function murong()
    ...
    local l = wait_line(var.go.path[var.go.path[msg].next].step, 30, nil, 20,
        "^穷光蛋，一边呆着去！$|"..
        "^你把钱交给船家，船家领你上了一条小舟。$|"..
        "^你随着船家上了一条小舟。$|"..
        "^你弹了一下琴，突然脚下一空，掉了下去。$|"..
...
function noweapon()
    message("info", debug.getinfo(1).source, debug.getinfo(1).currentline, "函数［ noweapon ］")
    local rc,msg = event_locate()
    if rc > 0 then
        return rc,msg
    end
    local l = wait_line(var.go.path[var.go.path[msg].next].step, 30, {StopEval=true}, 20,
        "^(\\S+)\\s+- |"..
        "^虚通迈步挡在你身前，双手合什说道：阿弥陀佛，这位\\S+请收起|"..
        "^虚明迈步挡在你身前，双手合什说道：阿弥陀佛，这位\\S+请收起|"..
        "^虚通拦住你说道：这位\\S+请放下兵刃。少林千年的规矩，外客|"..
        "^虚明拦住你说道：这位\\S+请放下兵刃。少林千年的规矩，外客|"..
        "^包不同拦住你说道：这位\\S+请放下兵刃。")
    if l == false then
        return noweapon()
    end
...
function duhe()
    message("info", debug.getinfo(1).source, debug.getinfo(1).currentline, "函数［ duhe ］")
    local rc,msg = event_locate()
    if rc > 0 then
        return rc, msg
    end
    local l = wait_line(var.go.path[var.go.path[msg].next].step, 30, nil, 20,
        "^(?:大渡口|陕晋渡口|澜沧江边|长江北岸|长江南岸) - |"..
        "^有船不坐，你想扮Cool啊？$|"..
    ...
    elseif var.go.pause ~= nil then
        return 1,"移动暂停"
    elseif l[0] == "江面太宽了，如果没有中途借力的地方根本没法飞越过去！" then
        wait(0.1)
        run("yell boat")
        return duhe()
    elseif l[0] == "有船不坐，你想扮Cool啊？" then
        repeat
            wait(0.1)
        until type(env.current.exits) == "table" and not set.has(env.current.exits, "enter")
        return duhe()
    elseif l[0] == "你的精力不够了！" then
        if yun_refresh() ~= 0 then
            map_adjust("过河", "渡船")
            return 1,"移动调整"
        end
        return duhe()
    elseif l[0] == "你的真气不够了！" then
        if dazuo() ~= 0 then
            map_adjust("过河", "渡船")
            return 1,"移动调整"
        end
        return duhe()
    elseif l[0] == "你的修为不够！" then
        map_adjust("过河", "渡船")
        return 1,"移动调整"
    else
        env.current.id = { var.go.path[msg].next }
...
function navigation()
    ...
    local l = wait_line("out", 30, {StopEval=true}, 20,
                        "^\\S+\\s+- |"..
                        "^你的眼前一黑，接著什么也不知道了....$|"..
                        "^你已经精疲力尽，动弹不得。$|"..
                        "^鬼门关 - $|"..
...
function kill_npc_exec()
    ...
    local l = wait_line(set.concat(var.go.kill_npc.step, ";"), 30, {StopEval=true}, 20,
        "^(\\S+)\\s+- |"..
        "^这个方向没有出路。$|"..
        "^什么\\?$|"..
        "^张松溪喝道：里面是武当重地，闲人请止步。$|"..
    ...
    elseif l[0] == "这个方向没有出路。" or l[0] == "什么?" then
        ...
        l = wait_line(var.go.kill_npc.kill,
                      ...
                      "^这里没有这个人。$|"..
                      "^(?:蒙面女郎|邱山风|司徒横|婢女|壮汉|张志光|皇宫卫士|公孙止|樊一翁|银甲侍卫|殷锦|一品堂武士|校尉|小兰|西华子|无根道人|日月神教卫士|谭处端|天鹰教守卫|说不得|石嫂|清乐比丘|清观比丘|蒙古军官|(?:男|女)教众|凌虚道长|王夫人|静心|戒律僧|江百胜|简长老|家丁|胡老爷|黄衫女子|公冶乾|葛伦布|宁中则|陆大有|岳灵姗|梁发|劳德诺|施戴子|高根明|都大锦|狮吼子|采花子|帮众|张松溪|丫鬟|慧空尊者|虚通|虚明|欧阳锋|一品堂武士|皇宫卫士|清观比丘)「啪」的一声倒在地上，挣扎着抽动了几下就死了。$|"..
                      "^你目前还没有任何为 移动暂停 的变量设定。$|"..
                      "^你的眼前一黑，接著什么也不知道了....$|"..
                      "^鬼门关 - $|"..
                      "^一道闪电从天降下，直朝你劈去……结果没打中！$")
